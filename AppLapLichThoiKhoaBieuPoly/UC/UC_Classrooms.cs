...
{
    public partial class UC_Classrooms : UserControl
    {
        DbConnection _KetNoi = new DbConnection();
        string _IDClass;
        public UC_Classrooms()
        {
            InitializeComponent();
        }
        private void UC_Classrooms_Load(object sender, EventArgs e)
        {
...
*/
        ...
        {
            string connectionString = _KetNoi.ConnectionString;
            string insertQuery = "INSERT INTO Classrooms (RoomName) VALUES (@RoomName)";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(insertQuery, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@RoomName", className);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_KetNoi.ConnectionString))
                {
                    ...
                    string deleteQuery = "DELETE FROM Classrooms WHERE RoomID = @RoomID";

                    // Tạo đối tượng Command và gán câu lệnh và kết nối cho nó
                    using (SqlCommand command = new SqlCommand(deleteQuery, connection))
                    {
                        // Thêm tham số cho câu lệnh SQL
                        command.Parameters.AddWithValue("@RoomID", _IDClass);
                        ...
                        if (rowsAffected > 0)
                        {
                            Console.WriteLine("Đã phòng học có RoomID = " + _IDClass);
                        }
                        else
                        {
                            Console.WriteLine("Không có phòng học nào được xóa.");
                        }
                    }
                    LoadDataClassrooms();
                    // Đóng kết nối
